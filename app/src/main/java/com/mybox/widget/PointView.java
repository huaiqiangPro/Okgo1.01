...
//
	...
	public int setPointCount(int count) {
		...
		return mScreenWidth - count * mImgMargin - mImgMargin;
	}
	...
	public void setCurrentIndex(int index) {
		mCurrentIndex = index == 0 ? 0 : (mAllCount == 0 ? 0 : index % mAllCount);
		postInvalidate();
	}
	...
	public void onDraw(Canvas canvas) {
		...
		int tImgMargin = mBitmapOn.getWidth()+10;

		int tStartLeft = (canvas.getWidth() - mAllCount * tImgMargin
				- tImgMargin);
		...
		for (int i = 0; i < mAllCount; i++) {
			...
			if (i == mCurrentIndex) {
				...
				int tLeftOn = (tStartLeft+ (i + 1) * tImgMargin)- mOnOffWidthDif;

				canvas.drawBitmap(mBitmapOn, tLeftOn, tTopOn, mPaint);
			} else {

				int tLeftOff = tStartLeft + (i + 1) * tImgMargin ;
